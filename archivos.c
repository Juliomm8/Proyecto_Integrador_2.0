...
void guardarInventario(const char *inventarioArchivo, struct Producto inventario[], int numProductos) {
  ...
  // El 'if' verifica si el archivo se pudo abrir, si no se pudo abrir, se
  ...
  for (int i = 0; i < numProductos; i++) {
    ...
    fprintf(archivo, "%s|%s|%s|%d|%.2f|%.2f|%d\n",
            ...
            inventario[i].requierePrescripcion); // Si requiere receta médica (1
                                                 // para sí, 0 para no)
  }
  ...
  printf("\n\033[1;32mInventario guardado exitosamente en la base de datos.\033[0m\n");
}
...
int cargarInventario(const char *inventarioArchivo, struct Producto inventario[]) {
  ...
  // fscanf devuelve el número de elementos leídos con éxito (esperamos leer 7
  // campos por línea)
  ...
  printf("\033[1;32mInventario cargado exitosamente desde la base de datos.\033[0m\n");
