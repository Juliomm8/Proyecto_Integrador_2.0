...
void ingresarProducto(struct Producto inventario[], int *numProductos) {
    ...
    do {
        ...
        getchar(); // Limpiar el buffer
        if (inventario[*numProductos].requierePrescripcion != 0 && inventario[*numProductos].requierePrescripcion != 1) {
            printf("\033[1;31mOpción no válida, ingresa 1 para Sí o 0 para No.\033[0m\n");
        }
    } while (inventario[*numProductos].requierePrescripcion != 0 && inventario[*numProductos].requierePrescripcion != 1);
